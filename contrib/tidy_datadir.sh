...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete VoodooCash database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no VoodooCash datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old VoodooCash datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected VoodooCash 0.7 datadir."
    ;;
  3)
    echo "Detected VoodooCash pre-0.8 datadir."
    ;;
  4)
    echo "Detected VoodooCash 0.8 datadir."
    ;;
esac
